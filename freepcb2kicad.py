...
class Library (object):
    ...
    def kicad_repr (self):
        ...
        if self.opts.blurb:
            lines.append ("# Automatically generated by freepcb2kicad")
            lines.append ("# c4757p.com/software/freepcb2kicad")
...
class PCBmodule (object):
    ...
    def kicad_repr (self):
        ...
        lines = []
        lines.append ("$Module " + self.Name)

        # WTF does Po stand for?
        # Oh... position.
        lines.append ("Po %d %d %d %d %08X %08X %s" %
                (0, # Xpos,
                    0, # Ypos,
                    0, # Orientation
                    15, # Layer (15 is the component layer)
                    int (time.time ()), # Timestamp - yes, not portable...
                    0, # Second timestamp? KiCad leaves it blank
                    "~~")) # Movable and not autoplaced
        ...
        # Reference
        lines.append ("T0 %d %d %f %f %d %f %s %s %d %s \"%s\"" %
                (0, # Xpos
                    0, # Ypos
                    TEXT_SIZE, # Xsize
                    TEXT_SIZE, # Ysize
                    0, # Rotation
                    TEXT_THICK, # Pen width
                    "N", # KiCad's shitty documentation doesn't even know what
                         # this means!
                    "V", # Visible
                    21, # Layer - front silk
                    "N", # Who knows?
        ...
        # Value
        lines.append ("T1 %d %d %f %f %d %f %s %s %d %s \"%s\"" %
                (0, # Xpos
                    0, # Ypos
                    TEXT_SIZE, # Xsize
                    TEXT_SIZE, # Ysize
        ...
        # 3D
        if self.ThreeDName is not None:
            lines.append ("$SHAPE3D")
            lines.append ("Na \"" + self.ThreeDName + "\"")
            lines.append ("Sc %f %f %f" % tuple (self.ThreeDScale))
            lines.append ("Of %f %f %f" % tuple (self.ThreeDOffset))
            lines.append ("Ro %f %f %f" % tuple (self.ThreeDRot))
            lines.append ("$EndSHAPE3D")

        lines.append ("$EndMODULE " + self.Name)
    ...
    def kicad_repr (self):
        ...
        if self.DrillDiam == 0:
            ...
            if self.opts.roundedpads is None:
                ...
                shape = "O" if can_round_pads else "R"
            elif self.opts.roundedpads == "allbut1":
                if can_round_pads:
                    shape = "R" if self.Name == "1" else "O"
                else:
                    shape = "R"
            else:
                assert False
            ...
            lines.append ("Sh \"%s\" %s %f %f %f %f %d" %
                    (self.Name,
                        ...
                        0.0,
                        0))
            lines.append ("Dr 0 0 0")
            lines.append ("At SMD N 00888000") # Layer mask
            lines.append ("Ne 0 \"\"") # Net reference
            lines.append ("Po %f %f" % (to_mm (self.Coords[0]),
                -to_mm (self.Coords[1])))
            ...
            lines.append ("Sh \"%s\" %s %f %f %f %f %d" %
                    (self.Name,
                        ...
                        0.0,
                        0))
            lines.append ("Dr %f 0 0" % to_mm (self.DrillDiam))
            lines.append ("At STD N 00E0FFFF") # Layer mask
            lines.append ("Ne 0 \"\"") # Net reference
            lines.append ("Po %f %f" % (to_mm (self.Coords[0]),
                -to_mm (self.Coords[1])))
...
def main (args=None, destfile=sys.stdout):
    from argparse import ArgumentParser
    description = "Read a FreePCB library file and convert it to Kicad " + \
            "format, with output to stdout. Uses the new millimeter " + \
            "format. If multiple files are given, they will be merged."
    ...
    blurbp.add_argument ("--blurb", dest="blurb", action="store_const",
            ...
            help="Include a blurb about freepcb2kicad in the output file's" +
            " comments (default: no)")
