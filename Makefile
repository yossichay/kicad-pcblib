...
.PHONY: all all-libraries check ipc download-ipc ipc-from-cache
...
ipcpretty-from-cache:
	rm -rf mod/IPC7351-Least.pretty
	mkdir mod/IPC7351-Least.pretty
	${PYTHON} download_ipc.py \
		--3dmap config/3dmap --rounded-pad-exceptions config/rpexceptions \
		${IPC_LEAST_NAME} mod/IPC7351-Least.pretty freepcb2pretty.py

	rm -rf mod/IPC7351-Most.pretty
	mkdir mod/IPC7351-Most.pretty
	${PYTHON} download_ipc.py --no-confirm-license \
		--3dmap config/3dmap --rounded-pad-exceptions config/rpexceptions \
		${IPC_MOST_NAME} mod/IPC7351-Most.pretty freepcb2pretty.py

	rm -rf mod/IPC7351-Nominal.pretty
	mkdir mod/IPC7351-Nominal.pretty
	${PYTHON} download_ipc.py --no-confirm-license \
		--3dmap config/3dmap --rounded-pad-exceptions config/rpexceptions \
		${IPC_NOMINAL_NAME} mod/IPC7351-Nominal.pretty freepcb2pretty.py
